...
이 문제는 트리의 성질을 알면 풀 수 있다.

타야 하는 비행기의 종류를 최소화 하는 문제, 즉 노드들을 연결하는 간선의 개수를 줄이는 문제이다.

모든 노드가 최소한의 간선 수로 연결되었을 때, 그래프는 트리가 되며, 필요한 간선의 수는 노드 수 $N$에서 1을 뺀, $N - 1$이다.